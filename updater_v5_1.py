...
def main():
    ...
    # Personal stocks are attempted first and retried individually.
    for sym, name in WATCH.items():
        try:
            x = clean(get_chart(sym))
            x["name"] = name
            stocks[sym] = x
            print(f"OK personal: {sym} ₹{x['last']}")
        except Exception as e:
            failures.append(f"{sym}: {e}")
            print(f"FAIL personal: {sym}: {e}")

    # Broader universe is best-effort.
    for sym, name in UNIVERSE.items():
        if sym in stocks:
            continue
        try:
            x = clean(get_chart(sym, attempts=2))
            x["name"] = name
            stocks[sym] = x
            print(f"OK universe: {sym}")
        except Exception as e:
            print(f"SKIP universe: {sym}: {e}")
